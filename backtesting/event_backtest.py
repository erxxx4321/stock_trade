...
class EventDrivenBacktester:
    ...
    def __init__(
        ...
        regime_filter=False,
        regime_graduated=False,
        regime_floor=0.30,
        gap_filter_atr=1.5,
        volume_confirm=False,
        blacklist_lookback=0,
        blacklist_min_wr=0.25,
        breakeven_pct=0,
        slippage=0,
        vol_parity=False,
    ...
    def _compute_atr(self, high_df, low_df, close_df, period=20):
        ...
        if isinstance(true_range, np.ndarray):
            true_range = pd.DataFrame(
                true_range, index=high_df.index, columns=high_df.columns
            )
    ...
    def _compute_regime_scale(
        self, i, dates, close_df, market_close, market_ma60, market_ma20
    ):
        ...
        if market_ma60 is not None:
            try:
                prev_date = dates[i - 1]
                mkt_date = market_close.index.get_indexer([prev_date], method="ffill")[
                    0
                ]
                if mkt_date >= 0:
                    ...
                    mkt_ma20 = (
                        market_ma20.iloc[mkt_date]
                        if market_ma20 is not None
                        else np.nan
                    )
        ...
        if self.macro_regime and self._vix_series is not None and regime_ok:
            try:
                prev_date = dates[i - 1]
                vix_idx = self._vix_series.index.get_indexer(
                    [prev_date], method="ffill"
                )[0]
                if vix_idx >= 0:
    ...
    def next_session_gap_limit(self):
        ...
        """
        if (
            getattr(self, "_dates", None) is None
            or getattr(self, "_close_df", None) is None
        ):
    ...
    def run(
        ...
        """
        mode_desc = (
            f"ATR×{self.tp_atr_mult}/{self.sl_atr_mult}"
        ...
        if self.macro_regime:
            try:
                ...
                if self._vix_series is not None:
                    print(
                        f"   🌍 Macro Regime: VIX 已載入 ({len(self._vix_series)} 天)"
                    )
        ...
        #         print(
        #             f"   📊 Sector Flow Tilt: {n_sectors} 板塊, "
        ...
        # print("regime_filter", self.regime_filter)
        # print("ma_close", market_close)
        ...
        for i in range(60, len(dates)):
            ...
            for ticker, trade in active_trades.items():
                ...
                if not pd.isna(current_high):
                    ...
                    if self.breakeven_pct > 0 and not trade.get(
                ...
                exit_reason = ""
                current_open = (
                    open_df[ticker].iloc[i] if ticker in open_df.columns else np.nan
                )
                ...
                if current_low <= trade["sl_price"]:
                    ...
                    try:
                        ticker_score = (
                            total_score[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                        )
            ...
            if self.regime_deleverage and market_ma60 is not None and active_trades:
                try:
                    prev_date = dates[i - 1]
                    mkt_date = market_close.index.get_indexer(
                        [prev_date], method="ffill"
                    )[0]
                    if mkt_date >= 0:
                        mkt_val = market_close.iloc[mkt_date]
                            ...
                            if regime_below_count >= 2:
                                ...
                                for ticker, trade in active_trades.items():
                                    ...
                                    if unrealized < -0.03:
                                        ...
                                        capital += revenue
                                        profit_pct = (
                                            exit_price_dv * (1 - self.sell_cost)
                                        ) / (
                                            trade["entry_price"] * (1 + self.buy_cost)
                                        ) - 1
                                        trades.append(
                                            {
                                                "Ticker": ticker,
                ...
                if regime_ok:
                    ...
                    for ticker in close_df.columns:
                        ...
                        prev_close = (
                            close_df[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                        )
                        ...
                        if vol_ma20 is not None and ticker in vol_df.columns:
                            prev_vol = (
                                vol_df[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                            )
                            avg_vol = (
                                vol_ma20[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                            )
                            if (
                                not pd.isna(prev_vol)
                    ...
                    for ticker in close_df.columns:
                        ...
                        ticker_rsi = (
                            rsi_14[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                        )
                        ticker_ret5 = (
                            ret_5d[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                        )
                        ma = ma_60[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                ...
                if current_sector_cap < 1.0:
                    ...
                    active_elec = sum(
                        1 for t in active_trades if t.startswith(elec_prefixes)
                    )
                ...
                if (
                    ...
                    and self._ma20_all is not None
                ):
                    try:
                ...
                if self.theme_breadth and len(candidates) >= 5:
                    try:
                        ...
                        elec_count = sum(
                            1 for t in top15 if str(t).startswith(elec_prefixes)
                        )
                ...
                if self.confidence_k and len(candidates) >= 3:
                    scores = [
                        c[1] for c in candidates[: min(top_k + 3, len(candidates))]
                    ]
                    top_score = scores[0]
                    if top_score > 0:
                ...
                if self.cluster_penalty and len(candidates) >= 3 and i >= 22:
                    try:
                        lookback = min(20, i)
                        cand_tickers = [
                            c[0] for c in candidates[: min(15, len(candidates))]
                        ]
                        held_tickers = list(active_trades.keys())
                        all_tickers = list(set(cand_tickers + held_tickers))
                        valid_tickers = [
                            t for t in all_tickers if t in close_df.columns
                        ]

                        if len(valid_tickers) >= 3 and lookback >= 10:
                            ret_slice = (
                                close_df[valid_tickers]
                                .iloc[max(0, i - lookback) : i]
                                .pct_change()
                                .dropna()
                            )
                            if len(ret_slice) >= 5:
                                ...
                                candidates = sorted(
                                    penalized, key=lambda x: x[1], reverse=True
                                )
                    except Exception:
                        pass
                    ...
                    # if (
                    #     self.sector_flow_tilt
                    #     and self._sector_flow_df is not None
                    #     and i - 1 >= 0
                    # ):
                    #     try:
                    #         from strategy.sector_flow import (
                    #             get_sector_slots,
                    #             select_with_sector_tilt,
                    #         )
                    ...
                    #                         pair_corr = (
                    #                             corr.loc[sel_tickers[si], sel_tickers[sj]]
                    #                             if sel_tickers[si] in corr.index
                    #                             and sel_tickers[sj] in corr.columns
                    #                             else 0
                    #                         )
                    ...
                    #                 if to_drop:
                    #                     selected = [
                    #                         s for s in selected if s[0] not in to_drop
                    #                     ]
                    #                     # 補上被過濾掉的名額
                    #                     remaining = [
                    #                         c
                    ...
                    #                         and c[0] not in to_drop
                    #                     ]
                    #                     selected += remaining[
                    #                         : min(top_k, slots_available) - len(selected)
                    #                     ]
                    #     except Exception:
                    #         pass
                    ...
                    if self.max_portfolio_heat < 1.0 and active_trades:
                        ...
                        for t_ticker, t_trade in active_trades.items():
                            t_price = (
                                close_df[t_ticker].iloc[i]
                                if not pd.isna(close_df[t_ticker].iloc[i])
                                else t_trade["entry_price"]
                            )
                    ...
                    if self.gap_aware_sizing and atr is not None:
                        ...
                        if (
                            not pd.isna(prev_close_val)
                            and not pd.isna(atr_val_gap)
                            and atr_val_gap > 0
                        ):
                    ...
                    if self.batch_entry > 1:
                        ...
                        batch_scale = weights.get(
                            self.batch_entry,
                            [1.0 / self.batch_entry] * self.batch_entry,
                        )[0]
                    ...
                    if self.dynamic_risk and market_daily_ret is not None:
                        try:
                            prev_date = dates[i - 1]
                            mkt_idx = market_close.index.get_indexer(
                                [prev_date], method="ffill"
                            )[0]
                            if mkt_idx >= 20:
                                recent_vol = market_daily_ret.iloc[
                                    mkt_idx - 20 : mkt_idx
                                ].std()
                                target_vol = 0.01  # 目標日波動 1%
                                if not pd.isna(recent_vol) and recent_vol > 0:
                                    vol_scalar = min(
                                        2.0, max(0.3, target_vol / recent_vol)
                                    )
                    ...
                    if self.vol_parity and atr is not None:
                        atr_val_sizing = (
                            atr[ticker].iloc[i - 1] if i - 1 >= 0 else np.nan
                        )
