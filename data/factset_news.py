...
class FactSetNewsDB:
    ...
    def insert_or_update(self, stock_code, stock_name, eps, est_price, date):
        """插入或更新資料"""
        self.cursor.execute(
            "SELECT stock_code FROM factset_news WHERE stock_code = ?", (stock_code,)
        )
        ...
        if exists:
            ...
            """
            self.cursor.execute(
                insert_sql, (stock_code, stock_name, eps, est_price, date)
            )
            action = "新增"
    ...
    def insert_from_news_title(self, title, date_str):
        ...
        return (
            True,
            f"{action} - {data['stock_code']} {data['stock_name']}: EPS={data['eps']}, 目標價={data['est_price']}",
        )
    ...
    def display_all(self):
        ...
        print("\n" + "=" * 120)
        print(
            f"{'股票代碼':<10} {'股票名稱':<15} {'EPS':<10} {'目標價':<10} {'日期':<20}"
        )
        print("=" * 120)
        ...
        for row in rows:
            ...
            print(
                f"{stock_code:<10} {stock_name:<15} {eps:<10.2f} {est_price:<10.2f} {date:<20}"
            )
...
def scrape_factset_news(
    keyword="Factset 最新調查", save_to_db=True, db_name="mystock.db"
):
    """
    爬取 Google News 上的 FactSet 新聞並儲存到資料庫
    """
    url = f"https://news.google.com/search?q=site:cnyes.com+{keyword}&hl=zh-TW&gl=TW&ceid=TW:zh-Hant"
    ...
    try:
        ...
        for container in news_containers:
            ...
            news_items.append(
                {"title": title, "date": date, "display_time": display_time}
            )
...
使用範例:
  ...
  python scrape_and_save.py --no-save  (只爬取不存資料庫)
    ...
    parser.add_argument(
        "--no-save", action="store_true", help="不儲存到資料庫，只顯示結果"
    )
    ...
    scrape_factset_news(
        keyword=args.keyword, save_to_db=not args.no_save, db_name=args.db
    )
